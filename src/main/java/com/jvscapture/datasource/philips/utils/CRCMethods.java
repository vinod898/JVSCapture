...
public class CRCMethods {
	// FCS calculation given in Network Working Group Request for Comment: 1171
	// (PPP protocol).
	// The one�s complement of the CRC is transmitted, rather than the CRC
	// itself

	public static int[] FCSTable = { 0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48, 0x9dc1,
	...
	public byte[] onesComplement(int numberToComplement) {
		...
		byte[] complementVal = BitConverter.getBytesU16(numberToComplement);
		List<Byte> complementBytes = new LinkedList<Byte>();

		for (int i = 0; i < complementVal.length; i++) {
			complementBytes.add((byte) (complementVal[i] ^ 0xff));
		}

		List<Byte> sublist = complementBytes.subList(0, 2);
